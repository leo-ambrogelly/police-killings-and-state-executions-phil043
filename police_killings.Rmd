...
author: "Léo L. Ambrogelly"
date: "`r Sys.Date()`"
output: pdf_document
---

The project below combines the graphs data from the Espy File (ICPSR 8451), the Death Penalty Information Center, the Fatal Encounters dataset, and the Mapping Police Violence dataset in order to graph executions and estimated police killings in the United States since 2000. 
...
executions <- bind_rows(executions_espy, dpic) %>%
  group_by(year) %>%
  summarise(executions = sum(executions), .groups = "drop")
...
ggplot(data_long, aes(year, deaths, colour = type)) +
  geom_line(size = 1) +
  scale_colour_manual(values = c("State executions"        = "firebrick",
